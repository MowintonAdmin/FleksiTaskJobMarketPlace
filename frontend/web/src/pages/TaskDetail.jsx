...
export default function TaskDetail() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // If verification pending or profile incomplete
      if (msg?.toLowerCase().includes('under review') || msg?.toLowerCase().includes('verification') || msg?.toLowerCase().includes('verified')) {
        navigate('/profile')
        toast.info(msg, { autoClose: 8000 })
      } else if (msg?.toLowerCase().includes('phone') || msg?.toLowerCase().includes('profile')) {
        navigate('/profile')
